...
namespace Avtozakupka
{
    public partial class Form4 : MetroForm
    {
        ...
        private void Form5_Load(object sender, EventArgs e)
        {
            ...
            // TODO: This line of code loads data into the 'baseDataSet.Заказ' table. You can move, or remove it, as needed.
            this.заказTableAdapter.Fill(this.baseDataSet.Заказ);
        }
        ...
        {
            for (int i = 0; i < заказDataGridView.Rows.Count - 1; i++)
            {
                заказDataGridView.CurrentCell = null;
                заказDataGridView.Rows[i].Visible = false;
                for (int c = 0; c < заказDataGridView.Columns.Count; c++)
                {
                    if (заказDataGridView[c, i].Value.ToString() == textBox1.Text)
                    {
                        заказDataGridView.Rows[i].Visible = true;
                        break;
                    }
                }
            }
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int colCount = заказDataGridView.ColumnCount;
            int rowCount = заказDataGridView.RowCount - 1;
            ...
            {
                if (g.MeasureString(заказDataGridView.Columns[current_col].HeaderText.ToString(), font).Width > widthC[current_col])
                {
                    widthC[current_col] = (int)g.MeasureString(заказDataGridView.Columns[current_col].HeaderText.ToString(), font).Width;
                }
                current_col++;
            }
            ...
            while (current_row < rowCount)
            {
                while (current_col < colCount)
                {
                    if (g.MeasureString(заказDataGridView[current_col, current_row].Value.ToString(), font).Width > widthC[current_col])
                    {
                        widthC[current_col] = (int)g.MeasureString(заказDataGridView[current_col, current_row].Value.ToString(), font).Width;
                    }
                    current_col++;
                }
                current_col = 0;
            ...
            int height = заказDataGridView[current_col, current_row].Size.Height;
            ...
            while (current_col < colCount)
            {
                ...
                cell_height = заказDataGridView[current_col, current_row].Size.Height;
                cell_border = new Rectangle(x, y, width, height);
                value = заказDataGridView.Columns[current_col].HeaderText.ToString();
            ...
            while (current_row < rowCount)
            {
                while (current_col < colCount)
                {
                    ...
                    cell_height = заказDataGridView[current_col, current_row].Size.Height;
                    cell_border = new Rectangle(x, y, width, height);
                    value = заказDataGridView[current_col, current_row].Value.ToString();
                    g.DrawRectangle(new Pen(Color.Black), cell_border);
                    g.DrawString(value, font, brush, x, y);
                    x += widthC[current_col]+ 150;
                    current_col++;
